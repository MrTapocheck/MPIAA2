...
#pragma omp parallel for num_threads(2)  //параллельное двухпоточное вычисление
   ...
   start_time = clock();
#pragma omp parallel for num_threads(16)    //параллельное шестнадцатипоточное вычисление
   for (int i = 0; i < n; i++) 
      for (int j = 0; j < n; j++)
         for (int k = 0; k < n; k++)
            C[i * n + j] += A[i * n + k] * B[k * n + j];
   ...
   cout << "Время параллельного двухпоточного вычисления: " << fixed << search_time16 << endl;


   norm = 0;
   for (int i = 0; i < (n * n); i++)
         norm += C[i] * C[i];
   cout << "Норма матрицы при использовании 16 потоков: " << sqrt(norm) << endl;
   
   cout << "Ускорение от двух потоков: " << search_time1 / search_time2 << endl;
   cout << "Ускорение от шестнадцати потоков: " << search_time1 / search_time16 << endl;
}
